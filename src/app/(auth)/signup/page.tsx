...
import PersonalPage from '@/components/auth/signup/PersonalPage';
import CompanyPage from '@/components/auth/signup/CompanyPage';
import ActivationPage from '@/components/auth/signup/ActivationPage';
    ...
    <Tabs dir={direction} defaultValue="personal" className="w-full">
      {step === 0 || step === 1 ? (
        <div className="flex flex-col items-center gap-2 text-center">
          ...
          <p className="w-full text-sm text-muted-foreground">{t('signupDescription')}</p>
        </div>
      ) : (
        <div className="flex flex-col items-center gap-2 text-center">
          <h1 className="text-2xl font-bold">{t('activateCode')}</h1>
          <p className="w-full text-sm text-muted-foreground">{descriptionText}</p>
        </div>
      )}
